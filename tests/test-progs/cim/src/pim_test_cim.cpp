...
int main(int argc, char* argv[])
{
    ...
    auto array1 = pim_alloc_safe<dtype>(N_ELEMS * sizeof(dtype), next_subarray);
    if (!array1) {
        printf("ERROR: pim_alloc_safe failed for array1\n");
        return 1;
    }

    auto array2 = pim_alloc_safe<dtype>(N_ELEMS * sizeof(dtype), next_subarray);
    if (!array2) {
        printf("ERROR: pim_alloc_safe failed for array2\n");
        pim_free(array1);
        return 1;
    }
