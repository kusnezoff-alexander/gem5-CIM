...
#define AAP_VECTORS(dst, src) FOR_ALL_ROWS_IN_VECTOR { rowclone(ROW(dst), ROW(src)); }
#define AND_VECTORS(dst, src) FOR_ALL_ROWS_IN_VECTOR { rowand(ROW(dst), ROW(src)); }
#define AP_VECTOR(dst)        FOR_ALL_ROWS_IN_VECTOR { rowmaj3(ROW(dst)); }
...
static unsigned **allocate_vector_array(size_t width, size_t height) {
	...
	if(NULL == (vector_array = (unsigned int **) malloc(height * sizeof(unsigned *)))) {
